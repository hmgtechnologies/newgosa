...
<script>
  ...
  renderNav(config, items, current) {
    return `<aside class="app-sidebar" id="app-sidebar">
      ...
      <nav class="app-nav">
        ${items.map(i => `<a href="${T.esc(i.href)}" data-role-allow="${T.esc(T.roleAllow(i.id))}" class="${i.href === (current || '').toLowerCase() + '.html' ? 'active' : ''}">
  ...
  roleAllow(id) {
    ...
    const map = {
      dashboard: all, academic_setup: admin, admin_data: admin, 'admin-data': admin, approvals: admin, storage: admin,
